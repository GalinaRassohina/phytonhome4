...
berries = list()
n = int(input("Веедите колличество кустов на грядке "))
for i in range(n):
    berries.insert(i, int(input("Введите число ягод на кусту: ")))
max = 0
for i in range(len(berries)):
    res = berries[i-1] + berries[i] + berries[i-len(berries)+1]
    if res> max:
        max = res
print(max)  